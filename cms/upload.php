<?php
	if(isset($_POST)){
		$nomearquivo = $_FILES['fleimage']['name'];
		$tamanhoarquivo = round(($_FILES['fleimage']['size']/1024));
		...
		if(in_array($extensao, $extensoes_permitidas)){
			if($tamanhoarquivo <= 5120){
				$arquivo_tmp = $_FILES['fleimage']['tmp_name'];

				if(move_uploaded_file($arquivo_tmp, $caminho_imagem)){
					echo '<img src="'.$caminho_imagem.'">';

					echo "<script>frmCms.txtfoto.value = '$caminho_imagem'</script>";
				}else{
