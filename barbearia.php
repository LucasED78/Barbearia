...
<html>
    ...
    <body>
        ...
        <div class="front">
            ...
            <div class="content">
              <?php
                $sql = 'SELECT * FROM tbl_sobre_barbearia WHERE status = 1 AND idSessao = 1';

                $resultado = mysqli_query($conexao,$sql);
                while ($rsBarbearia = mysqli_fetch_array($resultado)) {?>
                <section class="barbearia-servico">
                    <div class="row">
                        <div class="service-image">
                            <img src="cms/<?php echo (utf8_encode($rsBarbearia['imagem'])); ?>" alt="tatuagem">
                        </div>

                        <div class="service-desc">
                            <article>
                                <h2><?php echo(utf8_encode($rsBarbearia['titulo'])) ?></h2>

                                <p><?php echo(utf8_encode($rsBarbearia['descricao']))?></p>
                            </article>
                        </div>
                    </div>
                    <?php
                ...
                <section class="barbearia-atracao">
                    <?php
                        $sql = 'SELECT * FROM tbl_sobre_barbearia WHERE status = 1 AND idSessao = 2';

                        $resultado = mysqli_query($conexao,$sql);
                        while($rsBarbearia = mysqli_fetch_array($resultado)){?>
                    <div class="barbearia-atracao-item">
                        <img src="cms/<?php echo(utf8_encode($rsBarbearia['imagem'])) ?>" alt="atracao1">
